...
for a in applications:
    label = f"{a.get('client_name', 'Unknown Client')} | {format_money(a.get('loan_amount'))} | Score {a.get('score', 0)} | {a.get('workflow_status', 'UNKNOWN')}"
...
final_notes = st.text_area("Final Approval Notes", value=existing_final_notes, key=f"final_notes_{app['id']}")
decision_note = st.text_area("Approval / Rejection Note", value=existing_decision_note, key=f"final_note_{app['id']}")
...
with col1:
    if st.button("Approve", disabled=not is_pending_final_action, key=f"approve_final_{app['id']}"):
        ...
        if final_notes.strip() and "final_notes" not in app:
            history_note = f"Final Approval Notes: {final_notes.strip()}" + (f"\n\nDecision Note: {decision_note.strip()}" if decision_note.strip() else "")
        updated_history.append({"stage": role.upper(), "action": "APPROVED", "user": user.id, "timestamp": str(datetime.now()), "note": history_note})
        update_payload = build_safe_update_payload(app, {"workflow_status": "FINAL_APPROVED", "approval_history": updated_history, "final_notes": final_notes, "score": result["credit_score"], "decision": result["decision"], "credit_score": result["credit_score"], "risk_grade": result["risk_grade"], "risk_level": result["risk_level"], "dscr": result["dscr"], "collateral_cover": result["collateral_cover"], "borrower_summary": result["borrower_profile"], "facility_request": result["facility_details"], "risk_assessment": result["risk_assessment"], "decision_summary": result["decision_summary"], "ai_strengths": result["ai_strengths"], "ai_risk_flags": result["ai_risk_flags"], "ai_recommendation": result["ai_recommendation"], "ai_narrative": f"Credit Score: {result['credit_score']}/100 | Risk Grade: {result['risk_grade']} | DSCR: {result['dscr']:.2f}x | Decision: {result['decision']}", "final_review_by": user.id, "final_review_at": str(datetime.now())})
        supabase.table("loan_applications").update(update_payload).eq("id", app["id"]).execute()
...
with col2:
    if st.button("Reject", disabled=not is_pending_final_action, key=f"reject_final_{app['id']}"):
        ...
        if final_notes.strip() and "final_notes" not in app:
            history_note = f"Final Approval Notes: {final_notes.strip()}" + (f"\n\nDecision Note: {decision_note.strip()}" if decision_note.strip() else "")
        updated_history.append({"stage": role.upper(), "action": "REJECTED", "user": user.id, "timestamp": str(datetime.now()), "note": history_note})
        update_payload = build_safe_update_payload(app, {"workflow_status": "FINAL_REJECTED", "approval_history": updated_history, "final_notes": final_notes, "score": result["credit_score"], "decision": result["decision"], "credit_score": result["credit_score"], "risk_grade": result["risk_grade"], "risk_level": result["risk_level"], "dscr": result["dscr"], "collateral_cover": result["collateral_cover"], "borrower_summary": result["borrower_profile"], "facility_request": result["facility_details"], "risk_assessment": result["risk_assessment"], "decision_summary": result["decision_summary"], "ai_strengths": result["ai_strengths"], "ai_risk_flags": result["ai_risk_flags"], "ai_recommendation": result["ai_recommendation"], "ai_narrative": f"Credit Score: {result['credit_score']}/100 | Risk Grade: {result['risk_grade']} | DSCR: {result['dscr']:.2f}x | Decision: {result['decision']}", "final_review_by": user.id, "final_review_at": str(datetime.now())})
        supabase.table("loan_applications").update(update_payload).eq("id", app["id"]).execute()
        st.session_state.last_viewed_app = app["id"]
