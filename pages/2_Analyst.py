...
# LOAD APPLICATIONS
# ===============================
all_rows = supabase.table("loan_applications") \
    .select("*") \
...
applications = [
    ...
    if (row.get("workflow_status") or "") in allowed_statuses
]

if not applications:
    st.info("No applications available for analyst review.")
    st.stop()
...
for a in applications:
    label = (
        f"{a.get('client_name','Unknown')} - "
        f"{format_money(a.get('loan_amount',0))} - "
...
analyst_notes = st.text_area(
    "Analyst Notes",
    value=existing_analyst_notes,
    key=f"analyst_notes_{app['id']}"
)

decision_note = st.text_area(
    "Approval / Rejection Note",
    value=existing_decision_note,
    key=f"decision_note_{app['id']}"
)
